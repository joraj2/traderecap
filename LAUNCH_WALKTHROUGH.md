# EdgeBook — Launch Walkthrough

The single source of truth for getting EdgeBook from "code on disk" → "live on Play Store."
...
- `01c16c0` Initial commit — EdgeBook v1.0.0
...
| 2.2 | **You** | Apps → Add app → Android → Not on store yet → Name: EdgeBook → save the **App ID** |
...
| Step | Who | What |
|---|---|---|
| 6.1 | **You** | Run `keytool -genkeypair -v -keystore edgebook-release.jks -alias edgebook -keyalg RSA -keysize 2048 -validity 10000` → set + remember 2 passwords |
...
| **Splash screen (2732×2732 PNG)** | Centered EdgeBook logo on `#0a0d0c` background | required |
| **Feature graphic (1024×500 PNG)** | Banner with EdgeBook + tagline; use Canva or Figma | required |
...
| 8.2 | **You** | Create app: name "EdgeBook", default language en-US, App, Free, declarations all ticked |
...
**Outcome:** EdgeBook is publicly downloadable.
...
| Feature requests | Weekly triage | Email inbox for support@edgebook.app |
